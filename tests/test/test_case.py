...
class TestTestCaseClass(TestCase):
    ...
    def testSetMethodPatternStoresPattern(self) -> None:
        """
        Update _method_regex on TestCase when setMethodPattern is called.

        Validates that calling the classmethod with a custom glob pattern
        compiles a regex that matches names starting with 'check' but
        rejects names starting with 'test'.
        """
        try:
    ...
    def testSetMethodPatternDefaultIsTestStar(self) -> None:
        """
        Confirm the default regex matches names starting with 'test'.

        Validates that after resetting with 'test*' the compiled regex
        accepts methods like 'testSomething' and rejects others.
        """
        CoreTestCase.setMethodPattern("test*")
        self.assertIsNotNone(
            CoreTestCase._method_regex.match("testSomething"),
    ...
    def testSetMethodPatternAcceptsAnyString(self) -> None:
        """
        Accept an arbitrary non-empty string as the method pattern.

        Validates that setMethodPattern compiles whatever glob string is
        provided, updating _method_regex without raising an error.
        """
        try:
            CoreTestCase.setMethodPattern("verify_*")
            self.assertIsInstance(CoreTestCase._method_regex, re.Pattern)
            self.assertIsNotNone(
                CoreTestCase._method_regex.match("verify_something"),
            )
        finally:
            CoreTestCase.setMethodPattern("test*")
