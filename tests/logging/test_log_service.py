...
class TestLoggerMessages(TestCase):
    ...
    def testEmptyMessageIsIgnored(self) -> None:
        """
        Log an empty string without raising an exception.

        Validates that info("") completes without error. The Logger does not
        filter messages; an empty string is forwarded to the underlying
        logging system unchanged.
        """
        # Must not raise — Logger passes messages through without filtering
        self._logger.info("")

    def testWhitespaceOnlyMessageIsIgnored(self) -> None:
        """
        Log a whitespace-only string without raising an exception.

        Validates that info("   ") completes without error. The Logger does
        not strip messages before forwarding them to the logging system.
        """
        # Must not raise — Logger does not strip or discard messages
        self._logger.info("   ")
