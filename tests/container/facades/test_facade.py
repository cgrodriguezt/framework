...
class TestFacadeMeta(TestCase):
    ...
    def testGetAttrWithoutPinnedInstanceReturnsAsyncDispatcher(self) -> None:
        """
        Return an async dispatcher when no instance is pinned.

        The dispatcher is an async callable that resolves the service before
        forwarding the attribute access.
        ...
        self.assertTrue(inspect.iscoroutinefunction(dispatcher))
